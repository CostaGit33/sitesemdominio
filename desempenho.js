...
function renderDesempenho() {
  ...
  jogadoresArray.forEach((j, index) => {
    const percentual = Math.min((j.media / 16) * 100, 100);
    ...
    card.innerHTML = `
      ...
      <div class="media-badge" style="position: absolute; top: 10px; right: 10px; background: rgba(0,255,136,0.2); color: #00ff88; padding: 4px 8px; border-radius: 8px; font-weight: bold; font-size: 0.8rem;">${j.media.toFixed(1)}</div>
      ...
      <div class="player-stats" style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; width: 100%; font-size: 0.8rem; opacity: 0.8;">
        ...
        <span>Hab: ${j.stats[2]}</span>
        <span>Vel: ${j.stats[3]}</span>
      </div>
    `;
